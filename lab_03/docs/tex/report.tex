...
\begin{equation*}
	\frac{d}{dx}(\lambda(T)\frac{dT}{dx}) - 4 \cdot k(T) \cdot n_{\rho}^2 \cdot \sigma \cdot (T^4 - T_{0}^4) = 0
\end{equation*}\\
...
\indent $n_\rho$ = 1.4 - коэффициент преломления\\
...
\begin{equation*}
	f_{2}  = 4n^2_\rho \sigma \int^1_0 k(T(x))(T^4(x) - T^4_0) dx
\end{equation*}
...
\textbf{Представить разностный аналог краевого условия при $x = l$ и его краткий вывод интегро-интерполяционным методом.}
...
\textbf{График зависимости температуры $T(x)$ координаты $x$ при заданных выше параметрах.}\\

\textbf{График зависимости $T(x)$ при $F_{0}$ = -10 Bт / $cm^2$. }\\

\textbf{График зависимости $T(x)$ при увеличенных значениях $\alpha$ (например, в 3 раза). Сравнить с п. 2.}\\

\textbf{График зависимости $T(x)$ при $F_{0} = 0$.}\\

\textbf{Для указанного в задании исходного набора параметров привести данные по балансу энергии.}\\
...
\begin{equation*}
	\phi(y_{N})h + (k_{N} + \alpha_{N} h - k_{N} \xi_{N} - k_{N} \eta_{N})y_{N} - h\alpha_{N}T_{0} = 0
\end{equation*}
...
\textbf{4. Опишите алгоритм определения единственного значения сеточной функции $y_\rho$ в одной заданной точке $\rho$. Использовать встречную прогонку, т.е. комбинацию правой и левой прогонок.}\\
